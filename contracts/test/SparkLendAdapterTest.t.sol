...
contract SparkLendAdapterTest is Test {
    ...
    SparkLendAdapter public adapter;
    ...
    function setUp() public {
        ...
        // Deploy adapter and vault
        adapter = new SparkLendAdapter(SPARK_POOL);
        vault = new UniversalVault(UNISWAP_V3_ROUTER);

        // Add supported tokens to adapter
        vm.startPrank(adapter.owner());
        adapter.addSupportedToken(USDC, SPARK_AUSDC);
        adapter.addSupportedToken(DAI, SPARK_ADAI);
        vm.stopPrank();
    ...
    function testAddRemoveSupportedToken() public {
        ...
        assertEq(adapter.aTokens(USDC), SPARK_AUSDC);
    ...
    function testMultiTokenDeposit() public {
        ...
        // Both should be deposited into SparkLend
    ...
    function testGetAPYViaVault() public view {
        // Get APY via vault
        console.log("Getting APY via vault...");
    ...
    function testGetBalance() public {
        ...
        assertApproxEqRel(
            adapterBalance,
            vaultTotalBalance - IERC20(USDC).balanceOf(address(vault)),
            0.01e18
        ); // 1% tolerance
    }
